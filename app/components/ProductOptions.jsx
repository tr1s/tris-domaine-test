...
export function ProductOptions({productOptions}) {
  ...
  const sortedProductOptions = [...productOptions].sort((a, b) => {
    // Check if any option values have swatches
    const aHasSwatch = a.optionValues.some((value) => value.swatch);
    const bHasSwatch = b.optionValues.some((value) => value.swatch);

    if (aHasSwatch && !bHasSwatch) return -1;
    if (!aHasSwatch && bHasSwatch) return 1;
    return 0;
  });
